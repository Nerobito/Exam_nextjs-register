...
describe("RegisterDisaster contract", function () {
    ...
    beforeEach(async () => {
        ...
        // เปลี่ยนชื่อจาก "RegisterDisaster" เป็น "RegisterDisaster" ให้ตรงกัน
    ...
    describe("Get ID", () => {
        ...
        it("Should not find a person by ID if not registered", async () => {
            ...
            // เปลี่ยนข้อความที่คาดหวังให้ตรงกับข้อความที่ส่งคืน
            await expect(registerDisaster.getID(unregisteredID)).to.be.revertedWith("Person not found."); // เพิ่มจุดที่ท้ายข้อความ
        });
